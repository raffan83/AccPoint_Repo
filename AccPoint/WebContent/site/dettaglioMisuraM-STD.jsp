...
<c:forEach items="${righePerTabella}" var="entry">
  <c:set var="nomeTabella" value="${entry.key}" />
  <c:set var="righe" value="${entry.value}" />

<br>

  <!-- UM: la prendo dalla prima riga -->
  <c:set var="um" value="${(not empty righe) ? righe[0].um : 'g'}" />

  <div class="tab-wrapper">
    <table class="tab-pesi">
      <thead>
        <tr>
          <th>Valore nominale (${um})</th>
          <th>Valore delle pesate (${um})</th>
          <th>Valore corretto (${um})</th>
          <th>Valore convenzionale (${um})</th>
          <th>Scostamento (%)</th>
      ...
      <tbody>
        <c:forEach items="${righe}" var="r">
          <tr>
            <td>${r.valoreNominale}</td>
            <td>${r.pesata}</td>
            <td>${r.valoreCorretto}</td>
            <td>${r.valoreConvenzionale}</td>
            <td>${r.scostPct}</td>
            <td>${r.scostUm}</td>
            <td>${r.incPct}</td>
            <td>${r.incUm}</td>
...
.tabPM thead.pm-header th {
  ...
  table.tab-pesi { width:100%; border-collapse:collapse; font-family:Arial, Helvetica, sans-serif; font-size:12px; }
  .tab-pesi th, .tab-pesi td { border:2px solid #222; padding:6px 8px; text-align:center; vertical-align:middle; }
  .tab-pesi thead th { background:#d9d9d9; font-weight:700; }

.tab-wrapper { padding: 0 5px; } 
