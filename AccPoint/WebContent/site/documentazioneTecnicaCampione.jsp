...
<div class="row">
<div class="col-xs-12">
<h4>CARICAMENTO DOCUMENTAZIONE TECNICA</h4>
</div>
</div>
	<div class="row">
	<div class="col-xs-12">
...
<div class="col-xs-12">
		    ...
		    <div class="col-xs-3">
		        ...
		        <!-- The file input field used as target for the file upload widget -->
		        		<input accept=".PDF,.pdf,.xls,.xlsx,.doc,.docx,.jpg,.jpeg,.png" id="fileupload_documentazione" type="file" name="files">
		   	 </span>
...
<%-- </c:if>		
 ...
 <div class="col-xs-12">
 <table id="tabDocumentazione" class="table table-bordered table-hover dataTable table-striped" role="grid" width="100%">
 ...
 <c:forEach items="${lista_documentazione_tecnica}" var="documento" varStatus="loop">
 	<tr role="row" >
		...
		<a href="scaricaDocumentoEsternoCampione.do?action=scaricaDocumento&idDoc=${documento.id}" class="btn btn-info"><i class="fa fa-arrow-down"></i></a>
		<c:if test="${userObj.checkPermesso('LISTA_DOCUMENTI_ESTERNI_STRUMENTO_METROLOGIA')}">
...
<script src="plugins/fileSaver/FileSaver.min.js"></script>
 ...
 var columsDatatables = [];
	...
	$("#tabDocumentazione").on( 'init.dt', function ( e, settings ) {
	    ...
	    $('#tabDocumentazione thead th').each( function () {
	     	if(columsDatatables.length==0 || columsDatatables[$(this).index()]==null ){columsDatatables.push({search:{search:""}});}
	        var title = $('#tabDocumentazione thead th').eq( $(this).index() ).text();
	        if($(this).index()!= 0){
	...
	$('#fileupload_documentazione').fileupload({
        url: "scaricaDocumentoEsternoCampione.do?action=caricaDocumento&categoria=1",
        dataType: 'json',
        maxNumberOfFiles : 1,
        ...
        add: function(e, data) {
            var uploadErrors = [];
            var acceptFileTypes = /(\.|\/)(pdf|xls|xlsx|doc|docx|jpg|jpeg)$/i;
	...
	$('#fileupload_documentazione').bind('fileuploadsubmit', function (e, data) {
   ...
   var tableDocumenti = $('#tabDocumentazione').DataTable({
	   language: {
   ...
   tableDocumenti.buttons().container().appendTo( '#tabDocumentazione_wrapper .col-sm-6:eq(1)');
  ...
  // DataTable
	tableDocumenti = $('#tabDocumentazione').DataTable();
  // Apply the search
  ...
  tableDocumenti.columns.adjust().draw();
	...
	$('#tabDocumentazione').on( 'page.dt', function () {
