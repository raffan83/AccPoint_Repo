      ...
      <th style="text-align:center">
       <c:out value="<%=dayOfWeekString %>"></c:out>
        ...
        <tbody>
         ...
         <td id = "col_1_${utente.id}">
       		<b>${utente.nominativo}</b>
         </td>
...
.riquadro {
  ...
  word-break: break-word;     /* se un codice è lungo */
    ...
    $(document).off('input', '.inputsearchtable')
    .on('input', '.inputsearchtable', function() {
       var columnIndex = $(this).closest('th').index();
       table.column(columnIndex).search(this.value).draw();
    });

$(document).off('click', '.inputsearchtable')
    .on('click', '.inputsearchtable', function(e){ e.stopPropagation(); });
...
function modalPrenotazione(day, id_veicolo, id_prenotazione){
		...
		if(text!=null && text!='' && id_prenotazione!=null && id_prenotazione!=''){
			...
			callAjax(dataObj, "gestionePrenotazioneCampione.do?action=dettaglio_prenotazione", function(data){
...
function modalPrenotazione(day, id_veicolo, id_prenotazione){
  ...
  callAjax({ id: id_prenotazione }, "gestionePrenotazioneCampione.do?action=dettaglio_prenotazione", function(data){

    // anti-race: se nel frattempo ho cliccato altro, ignora
    if (_lastDetailRequestId !== String(id_prenotazione)) return;
    ...
    initializeTimepicker(
      prenotazione.data_inizio_prenotazione.split(" ")[1],
  ...
  //  if (windowScrollTop >= tableHeaderHeight) {
	  if (windowScrollTop > combinedHeaderHeight) {
        ...
        $('.DTFC_LeftHeadWrapper').css({
            'position': 'fixed',
            'top': '0',
            'left': $('.dataTables_scrollHead').offset().left,
 ...
 function fillTable(anno, filtro) {
	    ...
	    $.ajax({
	        ...
	        success: function(response) {
	        	...
	        	$("#tabPrenotazione").off('init.dt').on( 'init.dt', function ( e, settings ) {
	            ...
	            for (var i = 0; i < lista_prenotazioni.length; i++) {
	                var id_inizio = lista_prenotazioni[i].utente.id + "_" + lista_prenotazioni[i].cella_inizio;
	                var id_fine = lista_prenotazioni[i].utente.id + "_" + lista_prenotazioni[i].cella_fine;
		          ...
		          //  obj.id_veicolo = lista_prenotazioni[i].veicolo.id 
	                ...
	                if(posizionePartenza!=null){
	                	...
	                	for (var z = 0; z < lista_prenotazioni[i].listaCampioni.length; z++) {
	                	  codici.push(escapeHtml(lista_prenotazioni[i].listaCampioni[z].codice));
	                	}
	               ...
	               // 	 var testo = text + " (" + lista_prenotazioni[i].data_inizio_prenotazione.split(" ")[1] + " - " + lista_prenotazioni[i].data_fine_prenotazione.split(" ")[1] + ")";
	 	            ...
	 	            var maxLen = 0;
					for (var k=0;k<codici.length;k++) maxLen = Math.max(maxLen, codici[k].length);
					var larghezzaTesto = getTextWidth("X".repeat(maxLen), '12px Arial') + 20;
					...
					var altezza = Math.max(36, righe * lineHeight + paddingY);
	 	               ...
	 	               if(lista_prenotazioni[i].luogo!=null){
	 	                ...
	 	                if (numeroRiquadri === 0) {
	 	                  ...
	 	                  $("<div  data-toggle='tooltip' title='"+title+"' class='riquadro' id='riquadro_"+id_prenotazione+"' style='margin-top:42px;background-color:"+background_color+";border-color:"+border_color+"' ondblclick='modalPrenotazione("+id_inizio.split("_")[1]+", "+id_inizio.split("_")[0]+", "+id_prenotazione+")' >"+testo+ "</div>").addClass('riquadro') .css({
	 	                	    left: 0,
	 	                 ...
	 	                 $("<div data-toggle='tooltip' title='"+title+"'  class='riquadro' id='riquadro_"+id_prenotazione+"' style='margin-top:5px;background-color:"+background_color+";border-color:"+border_color+"' ondblclick='modalPrenotazione("+id_inizio.split("_")[1]+", "+id_inizio.split("_")[0]+", "+id_prenotazione+")' >"+testo+ "</div>").addClass('riquadro') .css({
	 	                    left: 0,
	 	                   top: 42,                 // <-- QUI controlli la distanza dall'header della cella
	 	                   width: larghezza,
	 	                   height: altezza,
