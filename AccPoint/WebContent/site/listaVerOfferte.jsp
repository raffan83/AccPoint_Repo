...
<th style="">Azioni</th>
 ...
 <tbody>
	...
	<tr id="row_${offerta.id }"  class="riga">
	<td>${offerta.id }</td>	
...
<td></td>
       ...
       <div class="row">
       	...
       	<div class="col-xs-4">
			<span class="btn btn-primary fileinput-button">
				...
				<input accept=".jpg,.png"  id="fileupload_img"  name="fileupload_img" type="file" multiple >
      ...
      <input type="hidden" id="id_offerta" name="id_offerta">
...
var righe =[];
 ...
 $('#nuovaOffertaForm').on('submit', function(e){
	 ...
	 tabArticoli.rows({ selected: true }).every(function () {
 	       ...
 	       id_articoli += id +","+quantita+ ","+sconto+";";
	 ...
	 if(id_articoli == ''){
		  $('#myModalErrorContent').html("Seleziona almeno un articolo!");
		  	$('#myModalError').removeClass();
			$('#myModalError').addClass("modal modal-danger");
 ...
 var allegati = [];
 var id_ordine_mobile =  Math.floor(Date.now() / 1000);
 
 async function preparaAllegati(files, id_ordine_mobile) {
	    ...
	    for (let file of filesArray) {
	     ...
	     const ext = file.name.split('.').pop().toLowerCase();
	        ...
	        all.push({
	            "FILE_TYPE": mimeType,     // tipo mimetype
	            "FILE_ATTACH": base64,         // base64 del file
	            "FILE_NAME": file.name,        // nome del file
	            "FILE_EXT": ext,             // estensione	          
	            "ID_ORDINE_MOBILE": id_ordine_mobile
	        });
	    }
 ...
 $('#fileupload_img').change( async function(){
	  ...
	  allegati = await  preparaAllegati(files);
	...
	function fileToBase64(file) {
	    return new Promise((resolve, reject) => {
	        ...
	        reader.onload = () => {
	            const result = reader.result.split(",")[1]; // prendi solo base64
	            resolve(result);
	        };
     ...
     if (type === 'row') {
         indexes.forEach(function(index) {
             ...
             $(row).find('td').each(function(i, cell) {
                 ...
                 if(i === 5){
                  	var id = $(row)[0].id;
                  	 const input = $('<input id="sconto_'+id+'" class="form-control" style="width:100%" type="number" min="0" step="1" value=""/>');
 	                        $cell.html(input);	
                  }
                  if(i === 4){
                 	var id = $(row)[0].id;
			...
			$('#importo_tot').val(offerta.importo);
		     ...
		     if(lista_articoli!=null){
		 		  for(var i = 0; i<lista_articoli.length;i++){
		 			 ...
		 			 if(lista_articoli[i].sconto!=null){
		 				 dati.sconto = lista_articoli[i].sconto;	
		 			 }else{
		 				 dati.sconto = "";	
		 			 }
...
function inviaOrdine() {
	...
	.then(data => {
	    if (data != null) {
	        var id_offerta = data.ID_OFFERTA;
