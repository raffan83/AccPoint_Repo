...
<div class="row">
<div class="col-xs-12">
<c:if test="${user.checkRuolo('AM') or user.checkRuolo('PP')}">
...
<th>Azioni</th>
 ...
 <tbody>
	...
	<td>${risorsa.partecipante.cognome }</td>	
	<td>${risorsa.utente.company.denominazione}</td>
	
	<td>

	<a class="btn btn-info customTooltip" onClicK="modificaRisorsaModal('${risorsa.id}',0)" title="Click per aprire il dettaglio della Risorsa"><i class="fa fa-search"></i></a> 
 	<c:if test="${user.checkRuolo('AM') or user.checkRuolo('PP')}">
 	<a  class="btn btn-warning" onClicK="modificaRisorsaModal('${risorsa.id}',1)" title="Click per modificare la risorsa"><i class="fa fa-edit"></i></a>
...
function modificaRisorsaModal(id_risorsa, type) {
    ...
    // reset stato modale prima del caricamento
    $('#id_risorsa').val(id_risorsa);

    $modal.find('.modal-title').text(solaLettura ? 'Dettaglio Risorsa' : 'Modifica Risorsa');
    $modal.find('button[type="submit"]').toggle(!solaLettura);

    $('#utente_mod').prop('disabled', solaLettura).trigger('change.select2');
    $('#partecipante_mod').prop('disabled', solaLettura).trigger('change.select2');

    // pulizia tabella sanitaria
    tableSan.rows().deselect();

    // blocco selezione checkbox tabella sanitaria in sola lettura
    if (solaLettura) {
        $('#tabRequisitiSanitari_mod tbody').css('pointer-events', 'none');
    } else {
        $('#tabRequisitiSanitari_mod tbody').css('pointer-events', '');
    }

    getRisorsa(id_risorsa, null, function(response) {
        ...
        $('#utente_mod').val(risorsa.utente.id).trigger('change');
        $('#partecipante_mod').val(risorsa.partecipante.id).trigger('change');
        ...
        // in modalità modifica riabilito i campi sanitari appena creati dinamicamente
        if (!solaLettura) {
            $('#tabRequisitiSanitari_mod')
...
$('#modificaRisorsaForm').on('submit', function(e){
 ...
 $('#modalModificaRisorsa').on("hidden.bs.modal", function(){
	    ...
	    $('#utente_mod').prop('disabled', false).trigger('change.select2');
	    $('#partecipante_mod').prop('disabled', false).trigger('change.select2');
	    ...
	    $(this).find('.modal-title').text('Modifica Risorsa');
