...
public class GestioneIntervento extends HttpServlet {
	...
	if(action !=null && action.equals("new")){
				...
				if(lat_master == null && check_nome_lat.equals("0") && (filename_pdf.startsWith("LAT") || nCertificato.startsWith("LAT"))) {
					...
					if(!non_sovrascrivere.equals("1")&&(note_obsolescenza==null||note_obsolescenza.equals("")) && isPresent) {
						...
						if(note_obsolescenza!=null && !note_obsolescenza.equals("")) {
							ArrayList<MisuraDTO> misuraObsoleta = GestioneInterventoDAO.getMisuraObsoleta(intervento.getId(),String.valueOf(strumento.get__id()),session);
		...
		else if(action !=null && action.equals("nuova_misura_cliente")) {
			...
			List<FileItem> items;
				...
				for (FileItem item : items) {
					if (item.isFormField()) {
						...
						if(item.getFieldName().equals("id_intervento")) {
							id_intervento = item.getString();
						}	
						else if(item.getFieldName().equals("id_strumento")) {
							id_strumento = item.getString();
						...
						if(item.getFieldName().equals("fileupload_certificato")) {
		    	...
		    	if(data_misura!=null && !data_misura.equals("")) {
		    		...
		    		certificato.setUtente(misura.getUser());
		    		if(file_pdf!=null && file_pdf.getName()!=null && !file_pdf.getName().equals("")) {
		    			certificato.setNomeCertificato(nomeFilePdfCertificato);	
		    		}
